...
Tai yra išorinės apsaugos sluoksnis - suteikti priėjimą prie saityno programų programavimo sąsajos prieigos taškų tik šios sistemos komponentams. Straipsnyje \cite{StormpathAPISecurity} apžvelgiami galimi apsaugos metodai ir išskiriamas saugiausias - \textit{OAuth 1.0a} protokolas.

Tai plačiai naudojamas, saugus, parašo pagrindu veikiantis protokolas. \textit{OAuth 1.0a} naudoja kriptografinį parašą, kuris susideda iš slapto prieigos rakto, vienkartinio rakto (angl. \textit{nonce}) ir kitos užklausos informacijos. Didelis pliusas šio protokolo - niekada tiesiogiai nesiunčiamas slaptas prieigos raktas, todėl nėra grėsmės, kad jis bus nuskaitytas siunčiant užklausą. Taip pat dėl šios priežasties nėra būtina \textit{SSL} apsauga. Kadangi \textit{OAuth 1.0a} yra plačiai naudojama, rakto generavimui yra sukurta nemažai įrankių, kurių dėka rakto generavimo procesas tampa paprastas.

\subsubsubsection{Sistemos duomenų apsauga}

Kuomet pasirūpinta, kad prieigos taškai yra prieinami tik šios sistemos komponentams, dar svarbu, kad skirtingos naudotojų grupės (mūsų atveju - dėstytojai ir studentai) galėtų pasiekti tik jiems skirtus išteklius. 
